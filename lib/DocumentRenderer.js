...
class DocumentRenderer {
  ...
  render (routingContext) {
    Promise.resolve()
      .then(() => {
        ...
        return state.signal(signal, routingContext, routingContext.args)
          .then((asyncActionResults) => {
            return { asyncActionResults, state };
          })
          .catch((error) => {
      ...
      .then(({ state, asyncActionResults }) => {
        var renderingContext = {
          state,
          routingContext,
          getHydrationScript: getHydrationScript.bind(null, asyncActionResults),
...
function getHydrationScript (asyncActionResults) {
  if (!asyncActionResults) {
    return '';
  }
  ...
  try {
    var hydrationValue = JSON.stringify(asyncActionResults);
