...
class ComponentReadable extends Readable {
  ...
  renderHTML (html) {
    ...
    while ((tokenDescriptor = tokenizer.next()).value !== null) {
      tokenDescriptor = tokenDescriptor.state === moduleHelper.ATTRIBUTE_NODE_TYPE ?
        Object.assign({
  ...
  _foundComponentHandler (tagDetails, parentComponent) {
    ...
    componentContext.parentComponent = this._rendereredComponentsList[parentComponent];
  ...
  _getComponentContext (context) {
    ...
    componentContext.props = this._getComponentProps(context);
  ...
  _getComponentProps (context) {
    const current = context.currentComponent || Object.create(null);

    const componentProps = current.props || Object.create(null);
    const parentPropsMap = current.parentPropsMap;

    if (typeof parentPropsMap === 'object') {
      const parent = context.parentComponent || Object.create(null);
      const parentProps = parent.props || Object.create(null);
  ...
  getHydrationScript () {
    ...
    try {
      let cache = JSON.stringify(this._context.asyncActionResults, (key, value) => {
        if (typeof value === 'string') {
          return moduleHelper.escape(value);
        }

        return value;
      });

      return `<script>var CATBEE_CACHE = ${cache}</script>`;
    } catch (e) {
...
function transformDescriptorToComponent (descriptor) {
  ...
  return {
    ...
    parentPropsMap: descriptor.parentPropsMap,
